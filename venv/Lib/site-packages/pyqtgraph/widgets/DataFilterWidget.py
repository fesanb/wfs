...
class RangeFilterItem(ptree.types.SimpleParameter):
	def __init__(self, name, opts):
		...
		ptree.types.SimpleParameter.__init__(self,
											 name=name, autoIncrementName=True, type='bool', value=True, removable=True,
											 renamable=True,
											 children=[
	...
	def generateMask(self, data, mask):
		...
		mask[mask] = (vals >= self['Min']) & (vals < self[
			'Max'])  ## Use inclusive minimum and non-inclusive maximum. This makes it easier to create non-overlapping selections
		return mask

	def describe(self):
		return "%s < %s < %s" % (
		fn.siFormat(self['Min'], suffix=self.units), self.fieldName, fn.siFormat(self['Max'], suffix=self.units))
