		...
		while True:
			try:
				...
				if err.errno == 4:  ## interrupted system call; try again
					...
					self.debugMsg('  handleRequest: got IOError %d from recv (%s); raise ClosedError.', err.errno,
								  err.strerror)
					raise ClosedError()
...
class ObjectProxy(object):
	...
	def __init__(self, processId, proxyId, typeStr='', parent=None):
		...
		self.__dict__['_handler'].registerProxy(
			self)  ## handler will watch proxy; inform remote process when the proxy is deleted.
